...
import { useResponsiveMonths } from "@/components/Events/useResponsiveMonths";
import { ServiceEventCellTooltip } from "./ServiceEventCellTooltip";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Skeleton } from "@/components/ui/skeleton";

// Utility: group events by date key
function getEventsByDate(events: ServiceEventWithService[]) {
  const byDate: Record<string, ServiceEventWithService[]> = {};
...
export const ServiceEventCalendar: React.FC<ServiceEventCalendarProps> = ({ 
  ...
  const [selectedEvents, setSelectedEvents] = React.useState<ServiceEventWithService[]>([]);
            ...
            {selectedEvents.map(event => (
              <div key={event.id} className="border-b border-muted pb-2 last:border-0">
                <ServiceEventCellTooltip 
                  event={event} 
                  serviceName={serviceMap[event.service_id] || "Unknown Service"} 
                />
              </div>
            ))}
          </div>
